...
(function () {
	 ...
	 *
	 * 20151029 EDIT - alxdnlnko
	 * 	Refactored directive:
	 * 	- renamed link function arguments according to Angular docs
	 * 	- renamed elem attributes to prevent conflicts with other libs
	 *	- added a $watch for scrollIf attribute to trigger the scrolling, when it changes
	 *	- removed unnecessary callbacks validations
	 *	- read some options from scope rather than attrs
	 */
	module.directive('smoothScroll', ['smoothScroll', function(smoothScroll) {
		return {
			...
			scope: {
				...
				scrollDuration: '=',
				scrollOffset: '=',
				scrollCallbackBefore: '&',
				scrollCallbackAfter: '&'
			},
			link: function(scope, elem, attrs) {
				if (typeof attrs.scrollIf === 'undefined') {
					// no scroll-if attribute, scroll once immediatelly
					startScrolling();
				} else {
					// scroll when scroll-if expression is true
					scope.$watch('scrollIf', function(val) {
						if (!!val) startScrolling();
					});
				}

				function startScrolling() {
					smoothScroll(elem[0], {
						duration: scope.scrollDuration,
						offset: scope.scrollOffset,
						easing: attrs.scrollEasing,
						callbackBefore: scope.scrollCallbackBefore,
						callbackAfter: scope.scrollCallbackAfter,
						containerId: attrs.scrollContainerId
					});
				}
			}
		};
	 ...
	 * 	- renamed link function arguments according to Angular docs
	 * 	- renamed elem attributes to prevent conflicts with other libs
	 * 	- early targetElement validation
	 *	- read some options from scope rather than attrs
	 *	- unsubscribe from events on element destroy
	 */
