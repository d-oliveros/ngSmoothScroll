	...
	var smoothScroll = function (element, options) {
		...
		setTimeout( function() {
			...
			if (options.stopOnUserScroll) {
				scrollableElements = container ? angular.element(container) : angular.element([document, window]);
			...
			var stopAnimation = function () {
				...
				var endCondition = container ? (container.offsetHeight + currentLocation) >= container.scrollHeight :
					(window.innerHeight + currentLocation) >= document.body.scrollHeight;

				if ( position == endLocation || currentLocation == endLocation || ( endCondition ) ) {
					if (options.stopOnUserScroll) {
						scrollableElements.off('wheel', userScrollListener);
					}
					clearInterval(runAnimation);
